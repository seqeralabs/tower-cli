...
def add_uge(
    ...
    head_queue: Annotated[
        str,
        typer.Option("-q", "--head-queue", help="Queue for launching execution of the Nextflow pipeline"),
    ],
    user_name: Annotated[
        Optional[str],
        typer.Option("-u", "--user-name", help="Username on the cluster used to launch the pipeline execution"),
    ] = None,
    host_name: Annotated[
        Optional[str],
        typer.Option("-H", "--host-name", help="Pipeline execution hostname for SSH connection (cluster login node). Local IP addresses (127.*, 172.*, 192.*, etc.) are not allowed, use a fully qualified hostname instead"),
    ] = None,
    port: Annotated[
        Optional[int],
    ...
    compute_queue: Annotated[
        Optional[str],
        typer.Option("--compute-queue", help="Queue for pipeline jobs submission. This queue can be overridden by the pipeline configuration"),
    ] = None,
    launch_dir: Annotated[
        Optional[str],
        typer.Option("--launch-dir", help="Directory where Nextflow runs. It must be an absolute directory and the user should have read-write access permissions to it [default: pipeline work directory]"),
    ] = None,
    max_queue_size: Annotated[
        Optional[int],
        typer.Option("--max-queue-size", help="Maximum number of jobs that Nextflow can submit to the UGE queue at the same time [default: 100]"),
    ] = None,
    head_job_options: Annotated[
        Optional[str],
        typer.Option("--head-job-options", help="UGE submit options for the Nextflow head job. These options are added to the submit command run by Tower to launch the pipeline execution"),
    ] = None,
    ...
    try:
        ...
        if not credentials_id:
            ...
            if not credentials:
                output_error("No SSH credentials found for UGE platform. Please create SSH credentials first.")
                sys.exit(1)
