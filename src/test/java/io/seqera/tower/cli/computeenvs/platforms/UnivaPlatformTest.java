...
class UnivaPlatformTest extends BaseCmdTest {
    ...
    void testAdd(OutputType format, MockServerClient mock) {
        ...
        mock.when(
                request().withMethod("POST").withPath("/compute-envs").withBody(
                        JsonBody.json("{\"computeEnv\":{\"credentialsId\":\"2ba2oekqeTEBzwSDgXg7xf\",\"name\":\"uge\",\"platform\":\"uge-platform\",\"config\":{\"userName\":\"jordi\",\"hostName\":\"ssh.mydomain.net\",\"workDir\":\"/home/jordeu/nf\",\"headQueue\":\"normal\"}}}")
                ),
                exactly(1)
        ).respond(
    ...
    void testAddWithAdvancedOptions(MockServerClient mock) {
        ...
        mock.when(
                request().withMethod("POST").withPath("/compute-envs").withBody(
                        JsonBody.json("{\"computeEnv\":{\"credentialsId\":\"2ba2oekqeTEBzwSDgXg7xf\",\"name\":\"uge\",\"platform\":\"uge-platform\",\"config\":{\"userName\":\"jordi\",\"hostName\":\"ssh.mydomain.net\",\"maxQueueSize\":200,\"workDir\":\"/home/jordeu/nf\",\"headQueue\":\"normal\"}}}")
                ),
                exactly(1)
        ).respond(
                response().withStatusCode(200).withBody("{\"computeEnvId\":\"isnEDBLvHDAIteOEF44ow\"}").withContentType(MediaType.APPLICATION_JSON)
