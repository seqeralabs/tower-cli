...
public class LaunchOptions {
    ...
    @Option(names = {"--revision"}, description = "Git commit ID, tag, or branch name")
    ...
    @Option(names = {"--entry-name"}, description = "Main workflow name to be executed when using DSL2 syntax")
    public String entryName;

    @Option(names = {"--schema-name"}, description = "Schema name")
    public String schemaName;

    @Option(names = {"--user-secrets"}, split = ",", description = "Pipeline secrets required by the pipeline execution that belong to the launching user personal context (user secrets take precedence over workspace secrets with the same name)")
    public List<String> userSecrets;

    @Option(names = {"--workspace-secrets"}, split = ",", description = "Pipeline secrets required by the pipeline execution (these secrets must be defined in the launching workspace)")
