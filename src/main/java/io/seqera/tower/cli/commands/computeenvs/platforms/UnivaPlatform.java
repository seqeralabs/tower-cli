...
public class UnivaPlatform extends AbstractPlatform<UnivaComputeConfig> {
    ...
    @Option(names = {"-q", "--head-queue"}, description = "Univa Grid Engine queue for launching the Nextflow head job. The queue where the main workflow orchestration process runs.", required = true)
    public String headQueue;

    @Option(names = {"--compute-queue"}, description = "Univa Grid Engine queue for pipeline task submission. Nextflow submits individual jobs to this queue. Can be overridden in pipeline configuration.")
    ...
    public UnivaPlatform() {
        super(PlatformEnum.UGE_PLATFORM);
    }

    @Override
    public UnivaComputeConfig computeConfig() throws ApiException, IOException {
        UnivaComputeConfig config = new UnivaComputeConfig();
    ...
    public static class AdvancedOptions {
        @Option(names = {"--max-queue-size"}, description = "Maximum number of jobs Nextflow can submit simultaneously to the Univa Grid Engine queue. Controls job submission rate. If absent, Platform defaults to 100.")
